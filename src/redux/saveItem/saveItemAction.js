...
const preloadWatchListTrue = () => {
  return {
    type: "PRELOAD_WATCHLIST_TRUE"
  };
};
  ...
  return (dispatch) => {
    ...
    if (ls !== null) {
      ...
      axios(`${BASE_URL}api/v1/User/FavoriteList?UserId=${userId}`)
        .then(
          (res) => res.status === 200 && dispatch(addToFavorite(res.data.data))
        )
...
const fetchingToSave = (userId, productId, lang) => {
  return (dispatch) => {
    ...
    if (ls !== null) {
      ...
      axios
        .post(
          `${BASE_URL}api/v1/User/AddToFavoriteList?UserId=${userId}&ProductId=${productId}`
        )
        .then((res) => {
...
const removeItem = (idItem, user) => {
  return (dispatch) => {
    ...
    axios
      .post(`${BASE_URL}api/v1/User/RemoveFromFavoriteList?id=${idItem}`)
      .then((res) => {
